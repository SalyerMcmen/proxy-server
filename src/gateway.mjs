...
// When enabled, unmodified upstream package tarballs are downloaded directly
// from npmjs.org. Registered custom packages continue to use the gateway.
const DIRECT_UPSTREAM_TARBALLS = process.env.DIRECT_UPSTREAM_TARBALLS === 'true';
...
function buildPackument(config, request, packageName, upstream, customPackage) {
  ...
  for (const [version, manifest] of Object.entries(upstreamVersions)) {
    ...
    versions[version] = {
      ...
      dist: {
        ...(manifest.dist ?? {}),
        tarball: DIRECT_UPSTREAM_TARBALLS && manifest.dist?.tarball
